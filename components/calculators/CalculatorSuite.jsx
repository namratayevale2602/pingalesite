...
function FutureWealth() {
  ...
  return (
    <div className="calc-grid grid grid-cols-1 gap-8 lg:gap-12 items-start lg:grid-cols-[380px_1fr]">
      <div>
        ...
        <div className="flex flex-col gap-5 mt-8 pt-6 border-t border-[var(--rule)] sm:flex-row sm:items-center sm:gap-8">
          <Donut invested={calc.totalInv} gains={calc.gains} size={120} />
          <div className="flex flex-col gap-3.5 min-w-0">
            ...
            <div className="pt-3 border-t border-[var(--rule)] font-[var(--font-display)] text-[clamp(22px,4vw,32px)]">
      ...
      <div>
        <div className="flex flex-wrap justify-between gap-2 mb-4 text-[12px] font-[var(--font-mono)] text-[var(--fg-muted)] tracking-[0.12em] uppercase">
  ...
  return (
    <div className="calc-grid grid grid-cols-1 gap-8 lg:gap-12 items-start lg:grid-cols-[380px_1fr]">
      <div>
        ...
        <SliderInput label="Returns During Earning Years"        value={earnPct}  onChange={setEarnPct}
          min={4} max={20} step={0.5} format={(v) => v.toFixed(1)} suffix="%" />
        <SliderInput label="Returns During Retirement Years"     value={retirPct} onChange={setRetirPct}
          min={4} max={15} step={0.5} format={(v) => v.toFixed(1)} suffix="%" />
        <SliderInput label="Current Wealth for Retirement"       value={wealth}   onChange={setWealth}
          min={0} max={50000000} step={100000} format={INR_FULL} />
      </div>
    ...
    <div className="calc-grid grid grid-cols-1 gap-8 lg:gap-12 items-start lg:grid-cols-[380px_1fr]">
      <div>
        ...
        <SliderInput label="Withdrawal Period"                       value={withdrawalPeriod} onChange={setWithdrawalPeriod}
          min={1} max={50} suffix="yrs" />
        <SliderInput label="Return During SIP Period"                value={sipReturn}        onChange={setSipReturn}
          min={4} max={18} step={0.5} format={(v) => v.toFixed(1)} suffix="%" />
        <SliderInput label="Return During Withdrawal Period"         value={swpReturn}        onChange={setSwpReturn}
          min={4} max={18} step={0.5} format={(v) => v.toFixed(1)} suffix="%" />
...
function GoalCalc() {
  ...
  return (
    <div className="calc-grid grid grid-cols-1 gap-8 lg:gap-12 items-start lg:grid-cols-[380px_1fr]">
      <div>
        <SliderInput label="Amount Required Today for My Goal" value={goalToday} onChange={setGoalToday}
        ...
        <SliderInput label="Investment Allocated for Goal"     value={currInv}   onChange={setCurrInv}
          min={0} max={100000000} step={100000} format={INR_FULL} />

        <div className="flex flex-col gap-5 mt-8 pt-6 border-t border-[var(--rule)] sm:flex-row sm:items-center sm:gap-8">
          <Donut invested={calc.futureValueOfCurrent} gains={Math.max(0, calc.futureGoalValue - calc.futureValueOfCurrent)} size={120} />
          <div className="flex flex-col gap-3.5 min-w-0">
      ...
      <div>
        <div className="grid grid-cols-1 gap-4 mb-6 min-[480px]:grid-cols-2">
  ...
  return (
    <div>
      ...
      <div className="flex gap-2 mb-6 flex-wrap">
        {CALC_LIST.map((c) => (
          <button key={c.id} onClick={() => setActive(c.id)}
            className="border-0 px-3 py-2 sm:px-4 sm:py-2.5 rounded-full font-[var(--font-sans)] text-[12px] sm:text-[13px] font-medium cursor-pointer whitespace-nowrap transition-colors"
            style={{
      ...
      <div className="mb-6 sm:mb-8">
        ...
        <h3 className="text-[clamp(16px,3.5vw,28px)] text-[var(--fg)] leading-snug">{subtitle}</h3>
      </div>
