    ...
    <section className="section bg-[var(--paper-2)] border-t border-b border-[var(--rule)]">
      <div className="container">
        <SectionHeader
          ...
          title={<>From conversation to <span className="italic-serif">cover</span> in days, not weeks.</>}
          align="split"
        />
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
          {steps.map((s, i) => (
            <div key={i} className="p-6 bg-[var(--bg-elev)] border border-[var(--rule)] rounded-2xl">
              <div className="font-[var(--font-display)] text-[48px] text-[var(--brand-2)] italic mb-4 leading-none">{s.n}</div>
