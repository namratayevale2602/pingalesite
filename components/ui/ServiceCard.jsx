...
export default function ServiceCard({ iconName, title, description, link, miniCalculator = false }) {
  ...
  return (
    <article className="group relative bg-white rounded-2xl p-6 shadow-sm border border-gray-100 hover:shadow-xl hover:-translate-y-1 transition-all duration-300 flex flex-col">
      ...
      <div className="absolute top-0 left-6 right-6 h-0.5 bg-[#a68256] rounded-full scale-x-0 group-hover:scale-x-100 transition-transform duration-300 origin-left" />

      {/* Icon */}
      <div className="w-12 h-12 rounded-xl bg-[#a68256]/10 flex items-center justify-center mb-5 group-hover:bg-[#a68256]/20 transition-colors duration-300">
        <Icon className="w-6 h-6 text-[#a68256]" />
      </div>
      ...
      {miniCalculator && (
        <div className="mt-5 p-4 bg-[#0F1A51]/4 rounded-xl border border-[#0F1A51]/10">
          <div className="flex items-center gap-2 mb-3">
            <Calculator className="w-4 h-4 text-[#0F1A51]" />
            <span className="text-xs font-semibold text-[#0F1A51] uppercase tracking-wide">
              Quick SIP Estimate
            </span>
          </div>
          ...
          <div className="flex gap-2 mb-2">
            <div className="relative flex-grow">
              ...
              <input
                ...
                className="w-full pl-7 pr-3 py-2 text-sm border border-gray-200 rounded-lg bg-white focus:outline-none focus:border-[#a68256] focus:ring-1 focus:ring-[#a68256]"
                placeholder="Monthly SIP"
              />
            </div>
            <button
              onClick={estimateSIP}
              className="bg-[#0F1A51] hover:bg-[#0F1A51] text-white text-xs font-semibold px-4 py-2 rounded-lg transition-colors whitespace-nowrap"
            >
              Estimate →
            </button>
          </div>
          ...
          {result !== null && (
            <p className="text-xs text-gray-600 mt-2">
              Est. value in 10 yrs:{" "}
              <span className="font-bold text-[#0F1A51]">
      ...
      <Link
        href={link}
        className="inline-flex items-center gap-1.5 text-sm font-semibold text-[#a68256] hover:text-[#0F1A51] mt-5 transition-colors group/link"
      >
        Learn More
